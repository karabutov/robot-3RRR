...
#        print("cr2: ", cr)
    ...
    def get_bcoors(self):
        return np.array([self.x0, self.y0, -math.pi, -math.pi, -math.pi, -math.pi])

    def get_ecoors(self):
        return np.array([self.x1, self.y1, math.pi, math.pi, math.pi, math.pi])

    def how_many_valid_size(self):
        if self.is_valid_size():
            return 1
        return 1 if self.s0 <= MAX_SIZE[0] else (round(self.s0 / MAX_SIZE[0]) + 1) * 1 if self.s1 <= MAX_SIZE[1] else (round(self.s1 / MAX_SIZE[1]) + 1)
    ...
    def print_box_only(self, head):
        print("" if head is None else head, "N: ", self.number, "Coors x:y ", self.x0, self.y0, self.x1, self.y1, "Size w:h", self.s0, self.s1, "Center x:y", self.centre_x, self.centre_y)
...
'''
    def split_dim(self, dim):
        if self.sizes[dim] < MAX_SIZE[dim]:
            return self, None
        cr = np.array(self.coordinates)
        cr[dim,1] = self.centres[dim]
        box1 = Box(cr)
        cr = np.array(self.coordinates)
        cr[dim,0] = self.centres[dim]
        box2 = Box(cr)
        return box1, box2
'''
