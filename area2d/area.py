...
def convolution(area):
    args_b = np.array([area[0][0], area[1][0], area[2][0], area[3][0], area[4][0], area[5][0]])
    args_e = np.array([area[0][1], area[1][1], area[2][1], area[3][1], area[4][1], area[5][1]])
...
def is_a_zero(func, minus_func, method, area):
...
def optimization(area):
    ...
    if not(is_a_zero(first_equation, minus_first_equation, "SLSQP",area)):
        if not(is_a_zero(first_equation, minus_first_equation, "L-BFGS-B", area)):
            return False

    if not(is_a_zero(second_equation, minus_second_equation, "SLSQP", area)):
        if not(is_a_zero(second_equation, second_equation, "L-BFGS-B", area)):
            return False

    if not(is_a_zero(third_equation, minus_third_equation, "SLSQP", area)):
        if not(is_a_zero(third_equation, minus_third_equation, "L-BFGS-B", area)):
            return False
...
def there_is_a_solution(aria, method):
    ...
    if abs(aria[0][0] - aria[0][1]) >= EPSILON[0] or abs(aria[1][0] - aria[1][1]) >= EPSILON[1]:
        return True
...
#        print("Arrays", res, cur_areas.shape)
        ...
        while(cur_areas.size != 0):
            ...
            if(issol):
                if abs(c_area[i][1] - c_area[i][0]) < EPSILON[i]:
